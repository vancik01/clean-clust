...
export async function placePodsAffinity(
    ...
    try {
        ...
        // Check if affinity is already set correctly
        ...
        const isAlreadySet = antiAffinity
            ...
            : existingNodeAffinity?.preferredDuringSchedulingIgnoredDuringExecution?.some(pref =>
                pref?.preference?.matchExpressions?.some(expr =>
                    expr.key === config.nodeSelectorName &&
                    expr.operator === 'In' &&
                    expr?.values?.includes(node)
                )
            );
        ...
        // Clear existing affinity only if it's different from the new one
        if (existingAffinity) {
            resetDeploymentAffinity(namespace, deploymentName)
        }

        // Define new affinity or anti-affinity
        const patchBody = {
            spec: {
                template: {
                    metadata: {
...
export async function getDeploymentNameForPod(namespace: string, pod: V1Pod): Promise<string | null> {
    // Get the ownerReferences from the pod metadata.
    const ownerRefs = pod.metadata?.ownerReferences;
    if (!ownerRefs || ownerRefs.length === 0) {
        return null;
    }

    // Find the ReplicaSet owner from the pod.
    ...
    try {
        ...
        if (rsOwnerRefs) {
            // Find the Deployment owner in the ReplicaSet's ownerReferences.
